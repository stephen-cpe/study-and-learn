...
The application features a retro-cyberpunk themed custom slide-deck engine with KaTeX math rendering, inline comprehension checkpoints, six quiz types (including ordering and matching), opt-in neural text-to-speech (TTS) narration, a gamified mascot personality engine with per-learner long-term memory, a notification bell for background tasks, document-grounded follow-up topic suggestions, source-figure thumbnails, and global content-addressable deduplication.
...
8. **Continue Learning:** Completed paths offer document-grounded follow-up topic suggestions that generate full modules on accept, and a notification bell reports finished background tasks with deep links so learners can multitask across tabs.
...
SG --> SGA{"Accept → generate module<br/>Dismiss → hide"}
SGA --> O
...
Narration carries learner memory: generation and retake pass the learner's recent memories into the narration prompt, which may reference at most one genuinely relevant fact (e.g. a passed module) in the intro. Narration prompts forbid LaTeX — mathematics is always spoken in plain words for text-to-speech.
...
After generation, the plan snapshot (`modules_json`, `summary_text`, `relevance_json` — never cleared, unlike session data and the digest) powers document-grounded follow-up suggestions: an LLM proposes up to three topics covered by the uploads but not yet taught or passed, skipping already-planned titles. Suggestions persist in a `Suggestion` table (pending/accepted/dismissed/completed) behind a "Keep Learning" card on the lessons page; accepting generates a full module through the standard lesson machinery (appended so sequential gating holds), dismissing hides it permanently. No web search is involved — external suggestions are a deferred phase.
