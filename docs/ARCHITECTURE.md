...
Final quizzes generate six questions — one per type: mcq, true/false, multi-select, cloze dropdown, ordering, and matching. Ordering uses reorder rows (position badge, ↑/↓ buttons, and ⠿ drag handles with finger-drag touch support; rank travels in hidden inputs pre-numbered 1–N), and matching uses pre-matched selects with swap buttons and drag-swap — placeholders are gone, so learners always see numbered starting values. A derangement guarantee in the shuffler means the display is never already solved: untouched 1,2,3,4 always grades wrong, so learners must rearrange something. Ordering and matching earn proportional credit (fraction of positions/pairs correct, shown as a partial verdict); everything else, including checkpoints, stays all-or-nothing. Per-question detail persists on the lesson so the lessons page can render a Weak Signals strip — missed or partial items with direct Review links (cleared on retake, since regenerated quizzes obsolete old rows). The deck renders LaTeX with KaTeX: slide prompts require `$…$`/`$$…$$` for formulas, the client stashes math spans before its lightweight markdown pass and renders after sanitization, and quiz inputs use custom retro radio/checkbox controls with unmistakable picked states (filled square for multi-select, matching the radio's filled circle).

### 5.9 Background Tasks & Notification Bell
Long tasks (`/process`, `/generate-lessons`) record durable rows in a `BackgroundTask` table (running → ready/failed with a result deep link) at every exit path, so any tab can track them via `GET /tasks`. A duplicate submit never starts a second pipeline: an insert-then-verify singleflight claim lets the oldest pipeline win while re-submits follow it (`resumed`) or are asked to wait (429), and the 3-active-lesson cap is re-checked at write time under claim. A navbar bell polls every ~15s, badges unread finished tasks, and offers per-row dismiss plus clear-finished (running rows are protected). Refresh resilience comes from persisted task IDs in `sessionStorage` plus `keepalive` POSTs — but resume is authoritative, never blind: the client only re-polls when the server still reports the task running, dead polls self-terminate, and a startup sweep flips rows orphaned by a restart to failed.
...
* **Unit Tests** verify isolated components such as file validation, document parsing, LangChain text splitting, prompt construction, AI client behavior, quiz generation (including reorder/derangement guarantees and partial credit), retrieval logic, text-to-speech utilities, spoken announcements, tutor-voice persona filtering, web-suggestion gating (classifier, sanitizer, verbatim URLs), background generation success/failure containment, and singleflight claims.
